...
void Common_Ancester(struct node* root,struct node* n){
     if(root==NULL){
          printf("NULL\n");
     }
     ...
     else if (n->data < root->data)
     {
          arr[i] = root->data;
          i++;
          Common_Ancester(root->left,n);
     }
     else if (n->data > root->data)
     {
          arr[i] = root->data;
          i++;
          Common_Ancester(root->right,n);
     }
}
...
int main()
{
     ...
     */
     ...
     struct node * n2 = root->left->right;
     printf("\nCommon A is : \n");
     Common_Ancester(root,n1);
     return 0;
}
